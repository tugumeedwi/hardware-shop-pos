-- ============================================================================
-- Cashier branch assignment
-- ----------------------------------------------------------------------------
-- Until now every till resolved to the tenant's default branch, so a cashier
-- working at a second shop still drained the head office's shelves. A
-- membership may now name the branch its member works at.
--
--   tenant_memberships.branch_id null  -> the tenant default (unchanged
--                                         behaviour for every existing row)
--   tenant_memberships.branch_id set   -> that member's till
--
-- create_sale is re-declared below to resolve the branch from the membership,
-- and to ENFORCE it for non-owners: a cashier's sale is always attributed to
-- their assigned branch, whatever branch_id the payload carries. That closes
-- the hole where a tampered client (or a sale queued offline before a
-- reassignment) could sell stock belonging to a different shop. Owners keep the
-- ability to name any branch of their own tenant, which is what the POS branch
-- switcher relies on.
--
-- Everything else in create_sale - idempotency, the tamper check, the
-- recalculation, the dual stock deduction, credit handling - is byte-identical
-- to 20260822000001; only the branch-resolution block and two new variables
-- differ.
-- ============================================================================
...
create index if not exists idx_tenant_memberships_branch
  on public.tenant_memberships(branch_id) where branch_id is not null;

-- A membership may only point at a branch of its own tenant. RLS already stops
-- a member from reading another tenant's branches, but an owner could otherwise
-- paste a foreign uuid straight into the column.
create or replace function public.memberships_validate_branch()
returns trigger
language plpgsql
security definer
...
begin
  if NEW.branch_id is not null then
    if not exists (
      ...
      raise exception 'Branch does not belong to this shop';
    end if;
  end if;
  return NEW;
end;
...
drop trigger if exists trg_memberships_validate_branch on public.tenant_memberships;
create trigger trg_memberships_validate_branch
  before insert or update of branch_id, tenant_id on public.tenant_memberships
  for each row execute function public.memberships_validate_branch();

-- The branch a given member works at, or null when unassigned.
create or replace function public.assigned_branch_id(p_tenant uuid, p_user uuid)
returns uuid
language sql
stable
security definer
...
grant execute on function public.assigned_branch_id(uuid, uuid) to authenticated;

-- The branch the CALLER should be selling from: their assignment, else the
-- tenant default. The frontend reads this to label the till.
...
as $$
  select coalesce(
    public.assigned_branch_id(public.get_my_tenant(), public.auth_uid()),
    public.default_branch_id(public.get_my_tenant())
  )
$$;
...
-- ----------------------------------------------------------------------------
-- create_sale: resolve and enforce the cashier's branch
-- ----------------------------------------------------------------------------
