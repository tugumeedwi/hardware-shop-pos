-- ============================================================================
-- Include per-product VAT in sale totals
-- ----------------------------------------------------------------------------
-- Previously the client showed a display-only tax line while the server
-- recalculated totals WITHOUT tax. That meant the receipt total disagreed with
-- what the tamper check stored. This redefines create_sale so the server
-- computes tax exactly as the client does:
--
--   tax = Σ line_total * (tax_rate / 100)
--   total = subtotal - discount + tax
--
-- For tenants/products with tax_rate = 0 (all existing catalogues) tax is 0 and
-- behaviour is unchanged. The client sends the tax-inclusive total_amount; the
-- tamper check now expects it to match.
-- ============================================================================
